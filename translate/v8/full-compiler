...
V8 实际上使用了两种不同的 JavaScript 编译器。我喜欢将它们想象成[简单编译器和辅助编译器](http://hyperboleandahalf.blogspot.com/2010/11/dogs-dont-understand-basic-concepts.html)。完整编译器(full compiler， 也就是本文的重点)是一个未经过优化的编译器。它的任务就是尽可能快的生成本地代码，保证页面迅速载入。Crankshaft 是经过优化的编译器。 V8 首先将所有内容使用完整编译器来编译，然后使用一个内置的分析器来选择 "hot" 函数用于 Crankshaft 编译。由于 V8 主要是单线程(自版本 3.14)，当编译器运行时，执行过程便会暂停。因此，这两个编译器都被设计成能够尽快的生成代码，而不会花费大量时间来生成搞非常高效的代码。在 V8 将来的版本中，Crankshaft (或至少其中的部分) 将会在一个单独的线程内运行，和 JavaScript 的执行并行开来，来实现更昂贵的优化。
...
无论是哪种情况，我们都需要解析源代码，生成一颗抽象语法树(AST)。我们需要执行范围分析，这将告诉我们每个符号指的是一个局部变量，上下文变量(用于闭包)，还是全局属性。翻译步骤是二者的唯一区别。你可以在这里做许多很精巧的事情，但如果你希望编译器变得更快，你需要一个非常直接的翻译：每一个语法树上的节点将被翻译为一条固定序列的字节码或本地指令。
...
Inline caches (ICs) provide an elegant solution to this problem. An inline cache is basically a function with multiple possible implementations (usually generated on the fly) which can be called to handle a specific operation. I previously wrote about [polymorphic inline caches for function calls](http://jayconrod.com/posts/44/polymorphic-inline-caches-explained). V8 uses ICs for a much broader set of operations: the full compiler uses ICs to implement loads, stores, calls, binary, unary and comparison operators, as well as the ToBoolean implicit operation.
